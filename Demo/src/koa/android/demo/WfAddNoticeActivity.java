 ...
 * 
 */
public class WfAddNoticeActivity extends Activity implements KoaActivity, OnGestureListener, OnTouchListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		boolean flag = LoginTimeOut.timeOut(WfAddNoticeActivity.this);
		if (!flag) {
			setContentView(R.layout.sendnext);
			mGestureDetector = new GestureDetector(this);
			// 获得参数
			intent = getIntent();
			sessionId = User.getInstance().getUserContext(WfAddNoticeActivity.this).getSessionId();
			// 初始化UI组件
			setUpView();
			// 增加组件监听器
			setUpViewListener();
	...
	public void setUpView() {
		...
		wf_sendnext_title_text.setText("传阅给:");
		setBackground();
	}
		...
		sender.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				...
				if (str.length() > 3) {
					...
					String senderUserJson = HttpResponseUtil.getInstance().getGetHttpInfo("KOA_Mobile_Get_UserName_By_Word", sessionId, userHash, WfAddNoticeActivity.this);
					if (null == senderUserJson) {
						Toast.makeText(WfAddNoticeActivity.this, "加载超时，请稍后再试", Toast.LENGTH_SHORT).show();
					} else if (senderUserJson.equals("TIMEOUT")) {
						Toast.makeText(WfAddNoticeActivity.this, "加载超时，请稍后再试", Toast.LENGTH_SHORT).show();
					} else if (senderUserJson.equals("NETWORK_ERROR")) {
						Toast.makeText(WfAddNoticeActivity.this, "网络连接异常，请稍后再试", Toast.LENGTH_SHORT).show();
					} else if (senderUserJson.equals("SESSION_TIMEOUT")) {
						HttpResponseUtil.getInstance().reLogin(WfAddNoticeActivity.this);
					} else {
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								ArrayAdapter<String> adapter = new ArrayAdapter<String>(WfAddNoticeActivity.this, R.layout.wf_help_inputuserid, temp);
	...
	class SenderBtnListener implements OnClickListener {
		public void onClick(View v) {
			pd = new ProgressDialog(WfAddNoticeActivity.this);
			...
			String sessionId = User.getInstance().getUserContext(WfAddNoticeActivity.this).getSessionId();
			HashMap<String, Object> userHash = new HashMap<String, Object>();
			userHash.put("userNames", checkSenderUser);
			String senderUserJson = HttpResponseUtil.getInstance().getGetHttpInfo("KOA_Mobile_Check_UserName", sessionId, userHash, WfAddNoticeActivity.this);
			if (senderUserJson == null && "".equals(senderUserJson)) {
				...
				if (result.equals("true")) {
					...
					Task task = new Task(Task.KOA_COMMON, "KOA_Mobile_SendReadTask", sessionId, "WfAddNoticeActivity", hashparam);
					// 将任务发送给Service来处理
					MainService.newTask(task);
					// 把自己添加到Activity集合里面
					MainService.addActivity(WfAddNoticeActivity.this);
				} else {
	...
	public void refresh(Object... params) {
		...
		if (json == null || "".equals(json)) {
			...
			intent.setClass(WfAddNoticeActivity.this, TodoListAllActivity.class);

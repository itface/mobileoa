...
import koa.android.logic.WfOtherAdapter;
...
public class noticeListActivity extends ListActivity {
	...
	private WfOtherAdapter adapter;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		boolean flag = LoginTimeOut.timeOut(noticeListActivity.this);
		if (!flag) {
			...
			String userName = User.getInstance().getUserContext(noticeListActivity.this).getUserName();
			top_title.setText(userName + " - 通知传阅");
			pd = new ProgressDialog(noticeListActivity.this);
	...
	public void setUpView() {
		list_footer = (LinearLayout) LayoutInflater.from(noticeListActivity.this).inflate(R.layout.list_footer, null);
	...
	public void setUpListeners() {
		title_bt_left.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				intent.setClass(noticeListActivity.this, MainActivity.class);
		...
		title_bt_right.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				MainService.promptExit(noticeListActivity.this);
			}
		});
		// 点击更多
			...
			public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
				...
				if (actionId == EditorInfo.IME_ACTION_UNSPECIFIED || actionId == EditorInfo.IME_ACTION_NONE || actionId == EditorInfo.IME_ACTION_DONE || actionId == EditorInfo.IME_ACTION_NEXT || actionId == EditorInfo.IME_ACTION_GO || actionId == EditorInfo.IME_ACTION_SEARCH) {
					...
					if (null != serachWords && !"".equals(serachWords)) {
						...
						Toast.makeText(noticeListActivity.this, "请输入搜索关键词", Toast.LENGTH_SHORT).show();
					}
				}
				return true;
			}
	...
	private void clearSearchAndConsumeList() {
		...
		adapter = new WfOtherAdapter(noticeListActivity.this, taskList);
	...
	public void refreshSearchResult(int pageNum, int pageSize) {
		...
		String sissionId = User.getInstance().getUserContext(noticeListActivity.this).getSessionId();// 获取sissionId
		if (sissionId != null || !"".equals(sissionId)) {
			...
			todolist_json = HttpResponseUtil.getInstance().getGetHttpInfo(socketCmd, sissionId, param, noticeListActivity.this);
		} else {
	...
	class GetTodoListAllHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (todolist_json == null || todolist_json.equals("")) {
				Toast.makeText(noticeListActivity.this, "加载超时,请稍后再试", Toast.LENGTH_SHORT).show();
			} else if (todolist_json.equals("TIMEOUT")) {
				Toast.makeText(noticeListActivity.this, "加载超时,请稍后再试", Toast.LENGTH_SHORT).show();
			} else if (todolist_json.equals("SESSION_TIMEOUT")) {
				HttpResponseUtil.getInstance().reLogin(noticeListActivity.this);
			} else if (todolist_json.equals("SESSION_ERROR")) {
				Toast.makeText(noticeListActivity.this, "加载超时,请稍后再试", Toast.LENGTH_SHORT).show();
			} else if (todolist_json.equals("NETWORK_ERROR")) {
				Toast.makeText(noticeListActivity.this, "网络连接异常，请稍后再试", Toast.LENGTH_SHORT).show();
			} else {
				...
				if (PAGE_NUM > 1) {
					if (taskList_page.size() > 0) {
						...
						Toast.makeText(noticeListActivity.this, "没有更多数据", Toast.LENGTH_SHORT).show();
					}
				} else if (PAGE_NUM == 1) {
					listView.setAdapter(adapter);
				}
				listView.setSelection((PAGE_NUM - 1) * PAGE_SIZE);// 设置最新获取一页数据成功后显示数据的起始数据
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		if (taskList != null) {
			if (id >= 0 && id < taskList.size()) {
				...
				intent.putExtra("fromActivity", "noticeListActivity");// 放入来源的Activity
				intent.setClass(noticeListActivity.this, WfReadActivity.class);
